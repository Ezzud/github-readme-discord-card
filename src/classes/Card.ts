...
export class CardOptions{
    ...
    constructor(
        usernameOrObj?: string | Partial<CardOptions>,
        displayName?: string,
        displayNameColor?: string,
        pfpImage?: string,
        height?: number,
        ...
        if (typeof usernameOrObj === 'object' && usernameOrObj !== null) {
            Object.assign(this, usernameOrObj);
        } else {
            this.username = usernameOrObj ?? this.username;
    ...
    render(): string {
        ...
        if(this.decorationFrameArray) {
            ...
            frames = this.decorationFrameArray.map((frame, index) => `
                ${index === 0 ? 
                        `
                        <image width="590" height="590" transform="translate(12 10) scale(0.13)" display='none' xlink:href="${frame}">
                            <animate 
                                    id='frame_${index}' 
